...
class TodoApplicationTest {
    ...
    void addTodo() {
        ...
        todoApp = new TodoApplication(Mock, personServiceMock);

        when(personServiceMock.findUsernameById(userID)).thenReturn("SomeUser");
        when(Mock.addTodo(userName, "Wake up")).thenReturn(true);
        ...
        verify(Mock, times(1)).addTodo(userName, "Sleep");
    ...
    void retrieveTodos() {
        ...
        TodoService retriveServiceMock = mock(TodoService.class);
        PersonService personServiceMock = mock(PersonService.class);
        todoApp = new TodoApplication(retriveServiceMock, personServiceMock);

        when(personServiceMock.findUsernameById(userID)).thenReturn(userName);
        when(retriveServiceMock.retrieveTodos(userName)).thenReturn(List.of("Test the code", "Celebrate the victory!"));
        ...
        verify(retriveServiceMock, times(1)).retrieveTodos(userName);

        verifyNoMoreInteractions(personServiceMock);
        verifyNoMoreInteractions(retriveServiceMock);
    ...
    void completeAllWithNoTodos() {
        ...
        when(Mock.findUsernameById(userID)).thenReturn(userName);
        when(todoServiceMock.retrieveTodos(userName)).thenReturn(List.of());

        todoApp.completeAllTodos(userID);

        verify(Mock, times(1)).findUsernameById(userID);
